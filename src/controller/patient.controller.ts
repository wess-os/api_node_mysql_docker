...
export const getPatients = async (req: Request, res: Response): Promise<Response<Patient[]>> => {
    ...
    try {
        ...
        return res.status(Code.INTERNAL_SERVER_ERROR).send(
            new HttpResponse(Code.INTERNAL_SERVER_ERROR, Status.INTERNAL_SERVER_ERROR, 'An error ocurred')
        );
    ...
    try {
        ...
        if ((result[0] as Array<ResultSet>).length > 0) {
            ...
            return res.status(Code.NOT_FOUND).send(
                new HttpResponse(Code.NOT_FOUND, Status.NOT_FOUND, 'Patient not found')
            );
